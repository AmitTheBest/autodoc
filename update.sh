...
if [ -f composer.json ]; then
    if [ -f composer.phar ]; then
        ...
        if find "composer.phar" -mmin +43200 >/dev/null; then
            echo "== STEP0 == Updating composer"
        ...
        CSS=""
        [ -x $HOME/Sites/agiletoolkit-css ] && CSS="$HOME/Sites/agiletoolkit-css"
        [ -x /var/www/agiletoolkit-css ] && CSS="/var/www/agiletoolkit-css"
        [ -x $HOME/www/agiletoolkit-css ] && CSS="$HOME/www/agiletoolkit-css"
...
echo " => Updating vendor/atk4/atk4"
